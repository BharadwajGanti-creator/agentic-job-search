...
public record RankedJob(string Title, string Company, string Location, string Link,
                        int Score, string Reason, string Missing);
public record SearchTrace(string[] PlannedQueries, int Fetched, int Unique, List<RankedJob> Ranked);
    ...
    public async Task<List<Job>> SearchAsync(string keywords, string location)
{
    ...
    return parsed.Jobs
        .Select(j => new Job(j.Title, j.Company, j.Location, j.Link, Clean(j.Snippet)))
        .ToList();
